...
class FontFitEditText : AppCompatEditText {
    // Attributes
    ...
    private fun refitText(text: String, textWidth: Int) {
        ...
        while (hi - lo > threshold) {
            val size = (hi + lo) / 2
            paint!!.textSize = size
            if (paint!!.measureText(text) >= targetWidth) hi = size // too big
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        refitText(Objects.requireNonNull(this.text).toString(), parentWidth)
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        if (w != oldw || h != oldh) {
            refitText(Objects.requireNonNull(this.text).toString(), w)
        }
    }
}
